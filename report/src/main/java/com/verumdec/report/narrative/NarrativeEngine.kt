...
class NarrativeEngine {
        ...
        val content = buildString {
            ...
            if (report.legalTriggers.isNotEmpty()) {
                ...
                for (trigger in report.legalTriggers.take(5)) {
                    appendLine("  • ${trigger.trigger}: ${trigger.description}")
                }
            }
        }
        ...
        val content = buildString {
            ...
            for ((entityId, entityAnomalies) in byEntity) {
                ...
                for (anomaly in entityAnomalies) {
                    appendLine("  • ${getBehavioralInsight(anomaly)}")
                }
    ...
    private fun generateForensicSummary(report: ContradictionReport): NarrativeSection {
        val content = buildString {
            ...
            appendLine("  • Total contradictions: ${report.totalContradictions}")
            appendLine("  • Timeline conflicts: ${report.timelineConflicts.size}")
            appendLine("  • Behavioral anomalies: ${report.behavioralAnomalies.size}")
            appendLine("  • Entities analyzed: ${report.affectedEntities.size}")
            appendLine("  • Documents reviewed: ${report.documentLinks.size}")
            appendLine()
            ...
            for ((severity, count) in report.severityBreakdown.entries.sortedByDescending { it.key }) {
                ...
                appendLine("  • $label ($severity): $count contradictions")
            }
            appendLine()
            ...
            appendLine()
            appendLine("---")
            appendLine("This report was generated by the Verum Omnis Contradiction Engine.")
            appendLine("Patent Pending - Verum Omnis")
        }
